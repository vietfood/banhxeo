...
This module teaches you how to decide which operations become which kernels.
This is one of the most important parts of a tensor compiler because it decides
which values stay in registers and which values become real memory.

## Why This Module Comes Before IR

The IR should represent a kernel body. Scheduling decides what a kernel body is.

If you design the IR first, you will either:

1. accidentally design an IR for the whole LazyBuffer graph, or
2. hide scheduling decisions inside the IR builder

Both are bad for learning. Keep the first separation sharp:

```text
scheduler: which LazyBuffers form one kernel?
IR builder: what does that one kernel compute?
renderer: how is that kernel written in Triton?
```

Do not chase optimal fusion yet. First make the current barrier policy explicit
and inspectable.
...
### Assignment 4.1: Understand banhxeo's Current Scheduling ⭐⭐
...
**Why this assignment:** You cannot improve fusion until you can predict the
current recursive execution order by hand.

---

### Assignment 4.2: Formalize Barrier Rules ⭐⭐
...
**Why this assignment:** A documented wrong rule is better than an implicit wrong
rule. Once the policy is named, tests can argue with it.

---

### Assignment 4.3: Understand tinygrad's Scheduling ⭐⭐⭐
...
**Why this assignment:** tinygrad is useful here because it shows the mature
version of the same problem. Read it to sharpen your questions, not to copy its
whole UOp universe.

---

### Assignment 4.4: Implement Fusion Analysis ⭐⭐⭐
...
**Why this assignment:** Fusion analysis is the first point where banhxeo becomes
a compiler rather than just a lazy interpreter with codegen.

---

### Assignment 4.5: Add Fusion Logging ⭐
...
### Assignment 4.6: Understand Advanced Fusion (Research) ⭐⭐⭐
